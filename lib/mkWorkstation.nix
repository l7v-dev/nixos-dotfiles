...
{ pkgs, lib ? pkgs.lib, inputs, homeManager, sops, host, user, system ? "x86_64-linux" }:
let
  hostDir = ./../hosts/${host};
  homeDir = ./../home;
in
lib.nixosSystem {
  ...
  modules = [
    ...
    { nixpkgs.pkgs = import pkgs { inherit system; config = { allowUnfree = true; permittedInsecurePackages = [ "librewolf-151.0.2-1" "librewolf-unwrapped-151.0.2-1" ]; }; }; }
    ({ config, pkgs, lib, ... }: {
