#!/usr/bin/env bash
# ==============================================================================
# AFT (Agentic Framework Template) - Project Initializer CLI
# ==============================================================================
# Kullanım:
#   ./scripts/aft-init.sh <proje-adi>
# ==============================================================================

set -euo pipefail

AFT_TEMPLATE_DIR="/home/l7v/dev/projects/company/active/nixos/templates/aft"
TARGET_BASE="/home/l7v/dev/projects/company/active"

if [[ $# -eq 0 ]]; then
  echo "❌ Hata: Lütfen bir proje adı belirtin."
  echo "Kullanım: $0 <proje-adi>"
  exit 1
fi
...
sed -i "s/\"name\": \"aft\"/\"name\": \"${PROJECT_NAME}\"/g" package.json
sed -i "s/PROJECT_NAME = \"aft\"/PROJECT_NAME = \"${PROJECT_NAME}\"/g" devenv.nix
sed -i "s/name: \"AFT\"/name: \"${PROJECT_NAME^^}\"/g" src/config/site.ts
