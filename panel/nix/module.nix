...
# support this extension without breaking the Phase 1 configuration.
{
  ...
  config,
  pkgs,
  # unstablePkgs is injected via specialArgs for both workstation and server
  # configurations. For workstations pkgs == unstablePkgs; for servers this
  # provides access to unstable-only build tools (e.g. fetchPnpmDeps).
  unstablePkgs ? pkgs,
  ...
}:
let
  ...
  # panel-agent uses gomod2nix (buildGoApplication) — needs the gomod2nix overlay,
  # which is present on both the workstation pkgs and the unstablePkgs set.
  panelAgentPkg = unstablePkgs.callPackage ./pkgs/panel-agent { };
  # panel-frontend uses fetchPnpmDeps which is only available in nixos-unstable.
  panelFrontendPkg = unstablePkgs.callPackage ./pkgs/panel-frontend { };
in
{
  # ── Options ────────────────────────────────────────────────────────────────
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf frontendCfg.enable {
      ...
      services.grafana.settings.security.allow_embedding =
        lib.mkIf config.l7v.services.grafana.enable true;
      ...
      systemd.services.panel-frontend = {
        ...
        serviceConfig = {
          Type = "simple";
          ExecStart = "${pkgs.nodejs_22}/bin/node ${panelFrontendPkg}/apps/web/server.js";
          WorkingDirectory = "${panelFrontendPkg}/apps/web";
          Restart = "on-failure";
          RestartSec = 5;
          StandardOutput = "journal";
          StandardError = "journal";
