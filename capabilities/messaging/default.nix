...
{ lib, config, pkgs, ... }:
{
  options.l7v.messaging = {
    ...
    smtp = {
      enable    = lib.mkEnableOption "SMTP relay (Postfix)" // { default = true; };
  ...
  config = lib.mkIf config.l7v.messaging.enable {
    ...
    services.postfix = lib.mkIf config.l7v.messaging.smtp.enable {
      enable   = true;
    ...
    services.postgresql.ensureDatabases =
      lib.optionals config.l7v.messaging.matrix.enable [ "matrix-synapse" ];

    services.postgresql.ensureUsers =
      lib.optionals config.l7v.messaging.matrix.enable [{
        name = "matrix-synapse";
        ensureDBOwnership = true;
      }];

    services.matrix-synapse = lib.mkIf config.l7v.messaging.matrix.enable {
      enable = true;
      settings = {
        ...
        registration_shared_secret_path =
          config.sops.secrets."matrix/registration_secret".path;
        enable_registration = false;
      };
    };

    sops.secrets."ntfy/auth_file" = lib.mkIf config.l7v.messaging.ntfy.enable {
      owner = "ntfy-sh";
    };

    services.ntfy-sh = lib.mkIf config.l7v.messaging.ntfy.enable {
      enable   = true;
      settings = {
