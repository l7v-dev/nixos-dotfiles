...
{ lib, config, pkgs, ... }:
{
  options.l7v.identity = {
    ...
    extraUsers = lib.mkOption {
      type    = lib.types.attrsOf (lib.types.submodule {
        options = {
          groups  = lib.mkOption { type = lib.types.listOf lib.types.str; default = [ "wheel" ]; };
  ...
  config = {
    ...
    i18n = {
      ...
      extraLocaleSettings = { LC_TIME = "tr_TR.UTF-8"; LC_MONETARY = "tr_TR.UTF-8"; };
    };
    ...
    users.users = lib.mkMerge [
      {
        ${config.l7v.identity.user} = {
          ...
          extraGroups  = [ "wheel" "networkmanager" "docker" "audio" "video" "input" "storage" "render" "kvm" ];
        };
      }
      (lib.mapAttrs (name: cfg: {
        ...
        extraGroups  = cfg.groups ++ lib.optionals cfg.isAdmin [ "wheel" "docker" ];
